...
def select_elements_starting_with_vowel(array)
	array.select {|e| e.match(/^[aeiouAEIOU]/)}
end
...
def array_sort_by_last_letter_of_word(array)
	array.sort_by {|element| element.chars.last }
end
...
def get_first_half_of_string(string)
	string.chars.shift((string.length/2.0).ceil).join
end
...
def number_of_elements_that_are_palindromes(array)
	array.select.count {|e| e.reverse == e }
end
...
def remove_capital_letters_from_string(string)
	string.chars.reject {|char| char[/[A-Z]/]}.join
end
...
def titleize_a_string(string)
	string.capitalize.scan(/\w+/).each { |word| word.capitalize! unless ['a', 'and', 'the'].include?(word) }.join(' ')
end
